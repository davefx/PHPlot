...
/////////////////////////////////////////////
    ...
    public function DrawMessage($text, $options = null)
    {
        ...
        if ($draw_background) {  // User-specified background
            ...
            ImageFilledRectangle($this->img, 0, 0, intval($this->image_width), intval($this->image_height), intval($bgcolor));
        }
        if ($draw_border) {
...
/////////////////////////////////////////////
        ...
        if (empty($this->done['background']) || $overwrite) {
            if (isset($this->bgimg)) {    // If bgimg is defined, use it
                ...
                ImageFilledRectangle(
                    ...
                    0,
                    0,
                    intval($this->image_width),
                    intval($this->image_height),
                    intval($this->ndx_bg_color)
                );
            }
            $this->done['background'] = true;
        }
    ...
    protected function DrawPlotAreaBackground()
    {
        if (isset($this->plotbgimg)) {
            ...
            ImageFilledRectangle(
                $this->img,
                intval($this->plot_area[0]),
                intval($this->plot_area[1]),
                intval($this->plot_area[2]),
                intval($this->plot_area[3]),
                intval($this->ndx_plot_bg_color)
            );
        }
        return true;
    }
        ...
        switch ($this->image_border_type) {
            case 'raised':
                ...
                for ($i = 0; $i < $width; $i++, $ex--, $ey--) {
                    imageline($this->img, intval($i ), intval($i ), intval($ex), intval($i ), intval($color1)); // Top
                    imageline($this->img, intval($ex), intval($i ), intval($ex), intval($ey), intval($color2)); // Right
                    imageline($this->img, intval($i ), intval($i ), intval($i ), intval($ey), intval($color1)); // Left
                    imageline($this->img, intval($i ), intval($ey), intval($ex), intval($ey), intval($color2)); // Bottom
                }
                break;
            ...
            case 'solid':
                for ($i = 0; $i < $width; $i++, $ex--, $ey--) {
                    imagerectangle($this->img, intval($i), intval($i), intval($ex), intval($ey), intval($color1));
                }
                break;
            default:
    ...
    protected function DrawXAxis()
    {
        ...
        if (!$this->suppress_x_axis) {
            ImageLine(
                $this->img,
                intval($this->plot_area[0] + 1),
                intval($this->x_axis_y_pixels),
                intval($this->plot_area[2] - 1),
                intval($this->x_axis_y_pixels),
                intval($this->ndx_grid_color)
            );
        }
        return true;
    }
    ...
    protected function DrawXTick($x, $x_pixels)
    {
        ...
        if ($this->x_tick_pos == 'xaxis') {
            ImageLine(
                $this->img,
                intval($x_pixels),
                intval($this->x_axis_y_pixels - $this->x_tick_cross),
                intval($x_pixels),
                intval($this->x_axis_y_pixels + $this->x_tick_length),
                intval($this->ndx_tick_color)
            );
        }
        ...
        if ($this->x_tick_pos == 'plotup' || $this->x_tick_pos == 'both') {
            ImageLine(
                $this->img,
                intval($x_pixels),
                intval($this->plot_area[1] - $this->x_tick_length),
                intval($x_pixels),
                intval($this->plot_area[1] + $this->x_tick_cross),
                intval($this->ndx_tick_color)
            );
        }
        ...
        if ($this->x_tick_pos == 'plotdown' || $this->x_tick_pos == 'both') {
            ImageLine(
                $this->img,
                intval($x_pixels),
                intval($this->plot_area[3] + $this->x_tick_length),
                intval($x_pixels),
                intval($this->plot_area[3] - $this->x_tick_cross),
                intval($this->ndx_tick_color)
            );
        }
    ...
    protected function DrawYTick($y, $y_pixels)
    {
        ...
        if ($this->y_tick_pos == 'yaxis') {
            ImageLine(
                $this->img,
                intval($this->y_axis_x_pixels - $this->y_tick_length),
                intval($y_pixels),
                intval($this->y_axis_x_pixels + $this->y_tick_cross),
                intval($y_pixels),
                intval($this->ndx_tick_color)
            );
        }
        ...
        if (($this->y_tick_pos == 'plotleft') || ($this->y_tick_pos == 'both')) {
            ImageLine(
                $this->img,
                intval($this->plot_area[0] - $this->y_tick_length),
                intval($y_pixels),
                intval($this->plot_area[0] + $this->y_tick_cross),
                intval($y_pixels),
                intval($this->ndx_tick_color)
            );
        }
        ...
        if (($this->y_tick_pos == 'plotright') || ($this->y_tick_pos == 'both')) {
            ImageLine(
                $this->img,
                intval($this->plot_area[2] + $this->y_tick_length),
                intval($y_pixels),
                intval($this->plot_area[2] - $this->y_tick_cross),
                intval($y_pixels),
                intval($this->ndx_tick_color)
            );
        }
    ...
    protected function DrawXTicks()
    {
        ...
        for ($n = 0; ($x = $x_start + $n * $delta_x) <= $x_end; $n++) {
            ...
            if ($draw_grid) {
                ImageLine($this->img, intval($x_pixels), intval($this->plot_area[1]), intval($x_pixels), intval($this->plot_area[3]), intval($style));
            }
    ...
    protected function DrawXDataLine($xpos, $row)
    {
        ...
        if ($this->x_data_label_pos == 'both') {
            ...
            ImageLine($this->img, intval($xpos), intval($this->plot_area[3]), intval($xpos), intval($this->plot_area[1]), intval($style));
            ...
            ImageLine($this->img, intval($xpos), intval($ypos), intval($xpos), intval($this->plot_area[3]), intval($style));
            ...
            ImageLine($this->img, intval($xpos), intval($this->plot_area[1]), intval($xpos), intval($ypos), intval($style));
        }
        return true;
    }
    ...
    protected function DrawYDataLine($ypos, $row)
    {
        ...
        if ($this->y_data_label_pos == 'both') {
            ...
            ImageLine($this->img, intval($this->plot_area[0]), intval($ypos), intval($this->plot_area[2]), intval($ypos), intval($style));
            ...
            ImageLine($this->img, intval($xpos), intval($ypos), intval($this->plot_area[0]), intval($ypos), intval($style));
            ...
            ImageLine($this->img, intval($this->plot_area[2]), intval($ypos), intval($xpos), intval($ypos), intval($style));
        }
        return true;
    }
...
/////////////////////////////////////////////
    ...
    protected function DrawLegend()
    {
        ...
        ImageFilledRectangle(
            ...
            intval($box_end_x),
            intval($box_end_y),
            intval($this->ndx_legend_bg_color)
        );
        if ($this->draw_legend_border) {
            ImageRectangle(
                ...
                intval($box_end_x),
                intval($box_end_y),
                intval($this->ndx_legend_border_color)
            );
        }
            ...
            if ($colorbox_mode) {
                ...
                switch ($colorbox_mode) {
                    ...
                    case 'line':
                        ...
                        imagesetthickness($this->img,intval($this->line_widths[$lws_index]));
                        $style = $this->SetDashedStyle(
                            $this->ndx_data_colors[$color_index],
                            $this->line_styles[$lws_index] == 'dashed'
                        );
                        imageline($this->img, intval($dot_left_x), intval($yc), intval($dot_right_x), intval($yc), intval($style));
                    ...
                    default:
                       ...
                       // Draw a rectangle around the box, if enabled.
                        if ($this->legend_colorbox_borders != 'none') {
                            ...
                            ImageRectangle($this->img,intval( $dot_left_x), intval($y1), intval($dot_right_x), intval($y2), intval($color));
                        }
                }
...
/////////////////////////////////////////////
    ...
    protected function DrawShape($x, $y, $record, $color, $allow_none = true)
    {
        ...
        switch ($this->point_shapes[$index]) {
            case 'halfline':
                ImageLine($this->img, intval($x1), intval($y), intval($x), intval($y), intval($color));
                break;
            case 'none': /* Special case, no point shape here */
                ...
                //no break
            case 'line':
                ImageLine($this->img, intval($x1), intval($y), intval($x2), intval($y), intval($color));
                break;
            case 'plus':
                ImageLine($this->img, intval($x1), intval($y ), intval($x2), intval($y ), intval($color));
                ImageLine($this->img, intval($x ), intval($y1), intval($x ), intval($y2), intval($color));
                break;
            case 'cross':
                ImageLine($this->img, intval($x1), intval($y1), intval($x2), intval($y2), intval($color));
                ImageLine($this->img, intval($x1), intval($y2), intval($x2), intval($y1), intval($color));
                break;
            case 'circle':
            ...
            case 'bowtie':
                ...
                break;
            case 'target':
                ImageFilledRectangle($this->img, intval($x1), intval($y1), intval($x), intval($y), intval($color));
                ImageFilledRectangle($this->img, intval($x), intval($y), intval($x2), intval($y2), intval($color));
                ImageRectangle($this->img, intval($x1), intval($y1), intval($x2), intval($y2), intval($color));
                break;
            case 'box':
                ImageRectangle($this->img, intval($x1), intval($y1), intval($x2), intval($y2), intval($color));
                break;
            ...
            case 'down':
                ...
                break;
            default: /* Also 'rect' */
                ImageFilledRectangle($this->img, intval($x1), intval($y1), intval($x2), intval($y2), intval($color));
                break;
        }
        return true;
    }
    ...
    protected function DrawBar(
        ...
        // Draw the bar
        ImageFilledRectangle($this->img, intval($x1), intval($y1), intval($x2), intval($y2), intval($data_color));
        ...
        if (isset($border_color)) {
            ...
            if ($y1 == $y2) {
                imageline($this->img, intval($x1), intval($y1), intval($x2), intval($y2), intval($border_color));
            } else {
                imagerectangle($this->img, intval($x1), intval($y1), intval($x2), intval($y2), intval($border_color));
            }
        }
    ...
    protected function DrawXErrorBars($x, $y, $error_plus, $error_minus, $color)
    {
        ...
        imagesetthickness($this->img,intval($this->error_bar_line_width));
        imageline($this->img, intval($x2p), intval($y1), intval($x2m), intval($y1), intval($color));
        if ($this->error_bar_shape == 'tee') {
            $e = $this->error_bar_size;
            imageline($this->img, intval($x2p), intval($y1 - $e), intval($x2p), intval($y1 + $e), intval($color));
            imageline($this->img, intval($x2m), intval($y1 - $e), intval($x2m), intval($y1 + $e), intval($color));
        }
        imagesetthickness($this->img,1);
        return true;
    }
    ...
    protected function DrawYErrorBars($x, $y, $error_plus, $error_minus, $color)
    {
        ...
        imagesetthickness($this->img,intval($this->error_bar_line_width));
        imageline($this->img, intval($x1), intval($y2p), intval($x1), intval($y2m), intval($color));
        if ($this->error_bar_shape == 'tee') {
            $e = $this->error_bar_size;
            imageline($this->img, intval($x1 - $e), intval($y2p), intval($x1 + $e), intval($y2p),intval( $color));
            imageline($this->img, intval($x1 - $e), intval($y2m), intval($x1 + $e), intval($y2m),intval( $color));
        }
        imagesetthickness($this->img,1);
        return true;
    }
    ...
    public function DrawAreaFillBorders($n_columns, $xd, $yd, $stacked, $stepped)
    {
        for ($col = 1; $col < $n_columns; $col++) {
            ...
            $x = $xd[0];
            $y = $yd[0][$this_col];
            ImageLine($this->img, intval($x), intval($yd[0][$other_col]), intval($x), intval($y), intval($color));
            ...
            for ($row = 1; $row < $this->num_data_rows; $row++) {
                ...
                $y = $yd[$row][$this_col];
                if ($stepped) {
                    ImageLine($this->img, intval($prev_x), intval($prev_y), intval($x), intval($prev_y),intval( $color));
                    ImageLine($this->img, intval($x     ), intval($prev_y), intval($x), intval(     $y), intval($color));
                } else {
                    ImageLine($this->img, intval($prev_x), intval($prev_y), intval($x), intval($y), intval($color));
                }
            }

            // Right edge:
            ImageLine($this->img, intval($x), intval($y), intval($x), intval($yd[$this->num_data_rows - 1][$other_col]), intval($color));
        }
        return true;
    }
...
/////////////////////////////////////////////
    ...
    protected function DrawThinBarLines()
    {
        ...
        for ($row = 0; $row < $this->num_data_rows; $row++) {
            ...
            for ($idx = 0; $rec < $this->num_recs[$row]; $rec++, $idx++) {
                if (is_numeric($dv = $this->data[$row][$rec])) {          // Allow for missing data
                    imagesetthickness($this->img,intval($this->line_widths[$idx]));
            ...
            for ($idx = 0; $record < $this->num_recs[$row]; $record++, $idx++) {
                if (is_numeric($y_now = $this->data[$row][$record])) {         // Allow for missing Y data
                    ...
                    if ($start_lines[$idx]) {
                        ...
                        // Draw the step:
                        $y_mid = $lasty[$idx];
                        ImageLine($this->img, intval($lastx[$idx] ), intval($y_mid), intval($x_now_pixels), intval($y_mid       ), intval($style));
                        ImageLine($this->img, intval($x_now_pixels), intval($y_mid), intval($x_now_pixels), intval($y_now_pixels), intval($style));
                    }
    ...
    protected function DrawOHLC($draw_candles, $always_fill = false)
    {
        ...
        for ($row = 0; $row < $this->num_data_rows; $row++) {
            ...
            imagesetthickness($this->img,intval($body_thickness));
            ...
            if ($draw_candles) {
                ...
                imagesetthickness($this->img,intval($wick_thickness));
                if ($yh_pixels < $yb1_pixels) {
                    imageline($this->img, intval($x_now_pixels), intval($yb1_pixels), intval($x_now_pixels), intval($yh_pixels), intval($ext_color));
                }
                if ($yl_pixels > $yb2_pixels) {
                    imageline($this->img, intval($x_now_pixels), intval($yb2_pixels), intval($x_now_pixels), intval($yl_pixels), intval($ext_color));
                }
            } else {
                // Basic OHLC
                imageline($this->img, intval($x_now_pixels), intval($yl_pixels), intval($x_now_pixels), intval($yh_pixels), intval($body_color));
                imagesetthickness($this->img,intval($wick_thickness));
                imageline($this->img, intval($x_left),  intval($yo_pixels), intval($x_now_pixels), intval($yo_pixels), intval($ext_color));
                imageline($this->img, intval($x_right), intval($yc_pixels), intval($x_now_pixels), intval($yc_pixels), intval($ext_color));
            }
            imagesetthickness($this->img,1);
    ...
    protected function DrawBoxes()
    {
        ...
        for ($row = 0; $row < $this->num_data_rows; $row++) {
            ...
            if (isset($yd[0]) && $yd[0] > $yd[1]) {   // Note device Y coordinates are inverted (*-1)
                imagesetthickness($this->img,intval($whisker_thickness));
                imageline($this->img, intval($xd          ), intval($yd[0]), intval($xd          ), intval($yd[1]), intval($whisker_style));
                imageline($this->img, intval($xd - $width2), intval($yd[0]), intval($xd + $width2), intval($yd[0]), intval($whisker_color));
            }

            // Draw the upper whisker and T
            if (isset($yd[4]) && $yd[3] > $yd[4]) {   // Meaning: Yworld[3] < Yworld[4]
                imagesetthickness($this->img,intval($whisker_thickness));
                imageline($this->img, intval($xd          ), intval($yd[3]), intval($xd          ), intval($yd[4]), intval($whisker_style));
                imageline($this->img, intval($xd - $width2), intval($yd[4]), intval($xd + $width2), intval($yd[4]), intval($whisker_color));
            }

            // Draw the median belt (before the box, so the ends of the belt don't break up the box.)
            if (isset($yd[2])) {
                imagesetthickness($this->img,intval($belt_thickness));
                imageline($this->img, intval($x_left), intval($yd[2]), intval($x_right), intval($yd[2]), intval($belt_color));
            }

            // Draw the box
            imagesetthickness($this->img,intval($box_thickness));
            imagerectangle($this->img, intval($x_left), intval($yd[3]), intval($x_right), intval($yd[1]), intval($box_color));
            imagesetthickness($this->img,1);
